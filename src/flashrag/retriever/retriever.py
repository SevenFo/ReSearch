...
def cache_manager(func):
    ...
    def wrapper(self, query=None, num=None, return_score=False):
        ...
        if self.save_cache:
            ...
            for new_query, doc_items, doc_scores in zip(
                query, save_results, save_scores
            ):
    ...
    def update_base_setting(self):
        ...
        if self.save_cache:
            self.cache_save_path = os.path.join(
                self._config["save_dir"], "retrieval_cache.json"
            )
            self.cache = {}
        if self.use_cache:
...
class BM25Retriever(BaseTextRetriever):
    ...
    def load_model_corpus(self, corpus):
        if self.backend == "pyserini":
            ...
            self.searcher = bm25s.BM25.load(
                self.index_path, mmap=True, load_corpus=False
            )
            if is_zh:
        ...
        def wrapper(*args, **kwargs):
            for i in range(max):
                try:
                    ...
                    if i == max - 1:
                        print(f"Retry {func.__name__} failed as {e} after {max} times")
                    elif sleep:
                        time.sleep(sleep)
...
class RemoteRetriever(BaseTextRetriever):
    ...
    def _search(self, query: str, num: int = None, return_score=False):
        ...
        response = requests.post(
            url, json={"query": query, "top_n": num, "return_score": return_score}
        )
...
class MultiModalRetriever(BaseRetriever):
    ...
    def __init__(self, config: dict, corpus=None):
        ...
        for modal in ["text", "image"]:
            ...
            if config["faiss_gpu"]:
                ...
                co.useFloat16 = True
                co.shard = True
                self.index_dict[modal] = faiss.index_cpu_to_all_gpus(
                    self.index_dict[modal], co=co
                )
        if corpus is None:
            self.corpus = load_corpus(self.corpus_path)
        else:
    ...
    def _search(
        ...
        query_modal = (
            self._judge_input_modal(query)
            if not isinstance(query, list)
            else self._judge_input_modal(query[0])
        )
    ...
    def _batch_search(
        ...
        for start_idx in tqdm(
            range(0, len(query), batch_size), desc="Retrieval process: "
        ):
            query_batch = query[start_idx : start_idx + batch_size]
            batch_emb = self.encoder.encode(query_batch, modal=query_modal)
            batch_scores, batch_idxs = self.index_dict[target_modal].search(
                batch_emb, k=num
            )
...
class MultiRetrieverRouter:
    def __init__(self, config):
        self.merge_method = config["multi_retriever_setting"].get(
            "merge_method", "concat"
        )  # concat/rrf/rerank
    ...
    def _search_or_batch_search(
        ...
        with ThreadPoolExecutor(max_workers=4) as executor:
            future_to_retriever = {
                executor.submit(process_retriever, retriever): retriever
            ...
            for future in as_completed(future_to_retriever):
                try:
                    ...
                    print(
                        f"Error processing retriever {future_to_retriever[future]}: {e}"
                    )
        result_list, score_list = self.reorder(result_list, score_list, retriever_list)
        result_list, score_list = self.post_process_result(
            query, result_list, score_list, num
        )
        if return_score:
    ...
    def reorder(self, result_list, score_list, retriever_list):
        ...
        for q_idx in range(query_num):
            final_result.append(
                sum(
                    [
                        result_list[q_idx + r_idx * query_num]
            ...
            if score_list != []:
                final_score.append(
                    sum(
                        [
                            score_list[q_idx + r_idx * query_num]
                            for r_idx in range(retriever_num)
                        ],
                        [],
                    )
    ...
    def post_process_result(
        ...
        if self.merge_method == "concat":
            ...
            if (
                isinstance(result_list[0], dict)
                and len(set([doc["corpus_path"] for doc in result_list])) > 1
            ) or (
                isinstance(result_list[0], list)
                and len(set([doc["corpus_path"] for doc in result_list[0]])) > 1
            ):
                warnings.warn(
    ...
    def batch_search(
        ...
        if all(query_type_list):
            ...
            if self.merge_method == "rerank":
                warnings.warn(
                    "merge_method is rerank, but all query is image, use default method `concat` instead"
                )
            ...
            if any(
                [
                    isinstance(retriever, BaseTextRetriever)
                    for retriever in self._retriever_list
                ]
            ):
                self._retriever_list = [
                    retriever
                    for retriever in self._retriever_list
                    if not isinstance(retriever, MultiModalRetriever)
                ]
            return self._search_or_batch_search(
                query,
                target_modal,
            ...
            if self.merge_method == "rerank":
                warnings.warn(
                    "merge_method is rerank, but some query is image, use default method `concat` instead"
                )
                self.merge_method = "concat"
