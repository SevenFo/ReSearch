...
from verl.utils.reward_score.re_search import (
    remove_boxed,
    ...
    def run_item(self, item):
        ...
        while remain_length > 0:
            ...
            if (
                ...
                stop_matched == 151643 or stop_matched == 151645
            ):
                output_str = response["text"]
                query += f"{output_str}"
                break
...
class IterativePipeline(BasicPipeline):
    def __init__(
        self, config, prompt_template=None, iter_num=3, retriever=None, generator=None
    ):
    ...
    def run(self, dataset, do_eval=True, pred_process_fun=None):
        ...
        for iter_idx in range(self.iter_num):
            if iter_idx == 0:
                ...
                input_query = [
                    f"{q} {r}" for q, r in zip(questions, past_generation_result)
                ]

            # generation-augmented retrieval
            retrieval_results = self.retriever.batch_search(input_query)
            dataset.update_output(
                f"retrieval_result_iter_{iter_idx}", retrieval_results
            )
...
class SelfRAGPipeline(BasicPipeline):
    ...
    def __init__(
        ...
        self.task_instruction = self.task_inst.get(
            self.task, self.task_inst["normal_qa"]
        )
        ...
        tokenizer = AutoTokenizer.from_pretrained(
            config["generator_model_path"], padding_side="left"
        )
    ...
    def load_special_tokens(self, tokenizer, use_grounding, use_utility):
        ret_tokens = {
            ...
            for token in self.retrieval_tokens_names
        }
        rel_tokens = {}
    ...
    def critic_preds(self, preds):
        ...
        for p_idx, pred in enumerate(preds):
            ...
            for tok, id in self.rel_tokens.items():
                prob = (
                    pred_log_probs[0][id].logprob if id in pred_log_probs[0] else -100
                )
            ...
            if self.grd_tokens is not None:
                ...
                if len(groundness_token_appear_indices) > 0:
                    ...
                    for token, token_id in self.grd_tokens.items():
                        prob = (
                            pred_log_probs[idx][token_id].logprob
        ...
        if "[No Retrieval]" in pred_text:
            ...
            for order, idx in enumerate(ret_token_appear_indices):
                ...
                for tok, tok_id in self.ret_tokens.items():
                    prob = (
                        pred_log_probs[idx][tok_id].logprob
            ...
            if curr_depth - 1 in levels and terminated is False:
                for node in levels[curr_depth - 1]:
                    ...
                    if "[Retrieval]" in pred:
                        ...
                        item_pred = self.generator.generate(
                            aug_prompts, return_raw_output=True, logprobs=5
                        )
                        _, preds, scores, overall_score_dict = self.critic_preds(
                            item_pred
                        )
    ...
    def run_batch_pred_long_form(self, dataset):
        ...
        for item, prompt, retrieval_flag in zip(
            dataset, input_prompts, retrieval_flags
        ):
            if retrieval_flag:
                pred, intermediate_result = self.run_single_beam(
                    prompt, item_retrieval_result=item.retrieval_result
                )
    ...
    def run_batch_pred(self, dataset):
        ...
        for idx, (prompt, item) in enumerate(zip(input_prompts, dataset)):
            ...
            if retrieval_flag:
                ...
                prompt_list = [
                    prompt
                    + "[Retrieval]<paragraph>{}</paragraph>".format(para["contents"])
        ...
        batch_pred = self.generator.generate(
            all_input_list, return_raw_output=True, logprobs=5
        )
        ...
        for idx, (retrieval_flag, item) in enumerate(zip(retrieval_flags, dataset)):
            if retrieval_flag:
                ...
                item_pred = batch_pred[
                    pred_idx : pred_idx + len(retrieval_results[idx])
                ]
...
class FLAREPipeline(BasicPipeline):
    ...
    def get_next_sentence(self, output, scores):
        ...
        if isinstance(tokenizer, (PreTrainedTokenizer, PreTrainedTokenizerFast)):
            token_id_sentences = [
                tokenizer.encode(s, add_special_tokens=False) for s in text_sentences
            ]
        else:
            token_id_sentences = [
                tokenizer.encode(s, allowed_special="all") for s in text_sentences
            ]
    ...
    def judge_sent_confidence(self, sent, sent_score):
        ...
        if not judge_result:
            ...
            new_query_ids = [
                i for i, score in zip(sent_ids, sent_score) if score > self.threshold
            ]
    ...
    def run_item(self, item):
        ...
        final_gen_result = ""
        while (
            gen_length < self.max_generation_length and iter_round < self.max_iter_num
        ):
            ...
            next_sent, next_sent_score = self.get_next_sentence(
                round_gen_output, scores
            )
            # judge next sentence
...
class SelfAskPipeline(BasicPipeline):
    ...
    def __init__(
        ...
        super().__init__(config, prompt_template)
        from flashrag.prompt.selfask_examplars import (
            SELF_ASK_PROMPT_SINGLE_HOP,
        ...
        self.P_INS = (
            SELF_ASK_PROMPT_SINGLE_HOP if self.single_hop else SELF_ASK_PROMPT_MULTI_HOP
        )
    ...
    def run_item(self, item):
        ...
        for idx in range(self.max_iter):
            ...
            gen_out = self.generator.generate(
                input_prompt, stop=["Context:", "#", stop_condition]
            )[0]
...
class IRCOTPipeline(BasicPipeline):
    ...
    def run_batch(self, items):
        ...
        retrieval_results, scoress = self.retriever.batch_search(
            questions, return_score=True
        )
        ...
        active_item_ids = list(
            range(len(items))
        )  # Track items that need more iterations
        while iter_num < self.max_iter:
            ...
            with ThreadPoolExecutor(max_workers=max_workers) as executor:
                future_to_prompt = {
                    executor.submit(
                        self.generator.generate, prompt, stop=[".", "\n"]
                    ): i
                    for i, prompt in enumerate(input_prompts)
                }
            ...
            if active_item_ids:
                new_thoughts_for_retrieval = [
                    batch_thoughts[item_id][-1] for item_id in active_item_ids
                ]
                new_retrieval_results, new_scoress = self.retriever.batch_search(
                    new_thoughts_for_retrieval, return_score=True
                )

                for i, item_id in enumerate(active_item_ids):
                    new_retrieval_result, new_scores = (
                        new_retrieval_results[i],
                        new_scoress[i],
                    )
                    ...
                    for doc_item, score in zip(new_retrieval_result, new_scores):
                        ...
                        if doc_id in doc2score_batch[item_id]:
                            doc2score_batch[item_id][doc_id] = max(
                    ...
                    sorted_doc_score = sorted(
                        doc2score_batch[item_id].items(),
                        key=lambda x: x[1],
                        reverse=False,
                    )
                    sorted_doc_id = [t[0] for t in sorted_doc_score]
                    batch_retrieval_results[item_id] = [
                        id2doc_batch[item_id][id] for id in sorted_doc_id
                    ]
...
class RQRAGPipeline(BasicPipeline):
    ...
    def generate_tree_of_thoughts_batch(self, initial_prompts_batch: List[str]):
        paths_batch_dict = {
            idx: [{"prompt": initial_prompt, "depth": 0, "done": False}]
        ...
        while any(paths for paths in paths_batch_dict.values()):
            ...
            for special_token in self.expand_on_tokens:
                if (
                    current_batch[0]["depth"] >= self.max_depth
                ...
                for i, decoded_output in enumerate(decoded_outputs):
                    ...
                    if special_token == "[A_Response]":
                        ...
                        result = (
                            matches[-1].strip()
                            if matches
                            else "Unable to detect valid answer"
                        )
                ...
                if queries_for_search:
                    batch_search_results = self.retriever.batch_search(
                        queries_for_search
                    )
        ...
        final_outputs_batch_list = [
            final_outputs_batch[i] for i in range(len(initial_prompts_batch))
        ]
    ...
    def run(self, dataset, do_eval=True):
        ...
        for i in tqdm(
            range(0, len(dataset), self.batch_size), position=0, desc="RQRAG Process"
        ):
            batch_items = dataset[i : i + self.batch_size]
            eval_datas = self.preprocess_eval_data(batch_items)
            paths_batch = self.generate_tree_of_thoughts_batch(
                initial_prompts_batch=eval_datas
            )
